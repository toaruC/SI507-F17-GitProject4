...
# https://apps.tumblr.com/ and creating an "app"
...
client_key = "" # what tumblr calls Consumer Key
...
def get_tokens():
    ...
    request_token_url = 'https://api.tumblr.com/oauth/request_token'
    ...
    # After the user authenticates at tumblr, it would normally "redirect"
    # the browser back to our website. But we aren't running a website.
    # Some services, like tumblr, will let you configure the app to
...
# For tumblr API endpoints that might be interesting to try, see docs at
# https://dev.tumblr.com/rest/tools/console and
# https://dev.tumblr.com/rest/public
...
# protected_url = 'https://api.tumblr.com/1.1/account/settings.json' # path that points to authenticated user's account settings data
...
r = oauth.get("https://api.tumblr.com/1.1/search/tweets.json", params = {'q': 'University of Michigan', 'count' : 3})

# Cache Data from Tumblr, searching for the phrase 'University of Michigan', looking to get 3 Tumblr back
try:
    tumblr_json = open('tumblr.json','r').read()
except:
    # tumblr_json = get_from_requests("http://newmantaylor.com/gallery.html", 'gallery.html')
    response = oauth.get("https://api.tumblr.com/1.1/search/tweets.json", params = {'q': 'University of Michigan', 'count' : 3})
    json = response.json
    f = open('tumblr.json', 'w')
    f.write(json)
    f.close
...
tumblr_data = json.loads(file_str)
print(type(tumblr_data)) # dictionary!

# remember the pattern for investigating nested data
print(list(tumblr_data.keys())) # [u'search_metadata', u'statuses']
# one of those looks interesting
print(type(tumblr_data["statuses"])) # print the type of that key's value
print(tumblr_data["statuses"][0]) # it's a list, so print the first element



####PAGING
# Getting multiple pages of results can be tricky. See tumblr's explanation at
# https://dev.tumblr.com/rest/public/timelines
...
# tumblr actually lets you get more than 25 at a time, but not more than 200, so this is practice for when you want to get more than 200. You could get the last 1000, 200 at a time, using the same pattern.
...
for i in range(5):
    if len(ids) > 0: # if we have already started the paging process
        ...
        # tumblr suggests that you take the minimum of the ids you got before, then subtract one from it, to make sure you get only ones you haven't received before. We can use the built-in min function here (could also accumulate by hand).
    # Regardless, now we need to make a request to the logged in user's timeline with the query parameters
    r = oauth.get("https://api.tumblr.com/1.1/statuses/user_timeline.json",params = my_params)  # passes {'count': 5, 'max_id': whatever} ...
...
# If you're testing with the data in the file only, you may want to comment out all the code above this for a while so you don't inadvertently make a lot of requests to tumblr and then run out of request privileges for the day!
